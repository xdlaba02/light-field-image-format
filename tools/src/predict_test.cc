...
int main(void) {
  ...
  for (size_t zz = 0; zz < 2 * BS + 2; zz++) {
      for (size_t y = 0; y < 2 * BS + 2; y++) {
        for (size_t z = 0; z < 2 * BS + 2; z++) {
        for (size_t x = 0; x < 2 * BS + 2; x++) {
          std::cout << std::setw(3) << test_block[((zz * (2 * BS + 2) + z) * (2 * BS + 2) + y) * (2 * BS + 2) + x];
        }
        std::cout << " ";
      }
      std::cout << std::endl;
  ...
  for (size_t zz = 0; zz < BS; zz++) {
      for (size_t y = 0; y < BS; y++) {
        for (size_t z = 0; z < BS; z++) {
        for (size_t x = 0; x < BS; x++) {
          std::cout << std::setw(3) << predicted_block[((zz * BS + z) * BS + y) * BS + x];
        }
        std::cout << ' ';
      }
      std::cout << std::endl;
