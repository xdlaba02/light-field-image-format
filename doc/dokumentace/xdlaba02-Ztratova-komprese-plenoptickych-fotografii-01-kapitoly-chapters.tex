...
V podkapitole \ref{jpeg} je podrobně popsán kompresní řetězec metody JPEG, který je dále rozvíjen v kapitole \ref{lfif-impl}.
\todo{Popsat zbytek kapitol, až budou.}


\chapter{Technologie plenoptických fotografií}
\label{pleno-teo}
Pro pochopení plenoptické fotografie je nejprve nutné pochopit pojem plenoptická funkce.
Tomuto pojmu se věnuje podkapitola \ref{pleno-fce}.


\section{Plenoptická funkce}
\label{pleno-fce}
Plenoptická funkce $P(x, y, z, \theta, \phi, \lambda, t)$ určuje intenzitu světelného paprsku v každém bodě v časoprostoru $x, y, z, t$ pro každou vlnovou délku $\lambda$ a pro každý úhel $\theta, \phi$.
Plenoptická funkce reprezentuje nejen všechno, co kdy bylo, je a bude viděno, ale také to, co by viděno být mohlo.

Jelikož není možné pozorovat realitu z každého bodu v časoprostoru, slouží plenoptická funkce pouze jako idealizovaný koncept.
Důležitost plenoptické funkce spočívá ve faktu, že svět je tvořen z objektů reálného světa, ktré své vlastnosti nepředávají jiným objektům přímo, ale pomocí světelných paprsků.
Tyto paprsky tvoří plenoptickou funkci.
Pozorovatel si může vzít vzorek z této funkce a na základě z něj interpretovat vlastnosti objektu.
